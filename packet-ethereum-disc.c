...
static void ethereum_discovery_stats_tree_init(stats_tree *st) {
  st_node_packets = stats_tree_create_node(st, st_str_packets, 0, 0, TRUE);
...
static void ethereum_srt_table_init(struct register_srt *srt _U_, GArray *srt_array,
                                    srt_init_cb gui_callback, void *gui_data) {
...
void proto_register_ethereum(void) {
  ...
  proto_ethereum = proto_register_protocol("Ethereum discovery protocol", "eth_discovery", "ethereum.disc");
...
void proto_reg_handoff_ethereum(void) {
  heur_dissector_add("udp", dissect_ethereum_heur, "Ethereum (devp2p) discovery", "eth_discovery",
